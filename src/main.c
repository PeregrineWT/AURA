        ...
        if (image_len > 0) {
            ...
            if (xSemaphoreTake(sd_card_mutex, portMAX_DELAY) == pdTRUE) {
                ...
                if (img_file != NULL) {
                    ...
                    // --- NEW: LOG THE CAMERA EVENT TO THE CSV ---
                    LogMessage_t cam_msg;
                    cam_msg.sensor_id = 'C';
                    cam_msg.timestamp = xTaskGetTickCount() * portTICK_PERIOD_MS;
                    cam_msg.values[0] = (float)image_counter; // Pass the counter through the float array
                    ...
                    switch (msg.sensor_id) {
                        ...
                        case 'B': 
                            fprintf(f, ",,,,,,,,,,,,,,,,,,,%lu,%.0f,%.0f,%.0f,%.0f\n", 
                                    msg.timestamp, msg.values[0], msg.values[1], msg.values[2], msg.values[3]);
                            break;
                        // --- NEW: THE CAMERA SPARSE MATRIX ENTRY (24 Commas) ---
                        case 'C':
...
void app_main(void) {
    ...
    if(spi_bus_initialize(SPI2_HOST, &buscfg, SPI_DMA_CH_AUTO) == ESP_OK) {
        spi_device_interface_config_t devcfg = {
            ...
            .mode = 0,                 
            .spics_io_num = -1, // CS handled manually by Arducam
            .queue_size = 7,
        };
